...
def _resolve_active_sprint_for_user(user_id: str) -> dict | None:
    ...
    Strategy (no .ilike() — Python-side string filtering everywhere):
      ...
      2. Find sprint(s) that the user has assigned tasks in AND that are active.
         Only looks at NON-CCR tasks (mid_sprint_changed=false) so a previously
         created CCR task with a sentinel sprint_id doesn't poison this lookup.
    ...
    # We exclude mid_sprint_changed=true tasks because CCR tasks may carry a
    # sentinel sprint_id (e.g. ffffffff-fe00-...) that does not exist in the
    # sprints table, causing the active-sprint lookup to return nothing and
    # fall through to a wrong sprint.
    task_res = (
        db.table("tasks")
        .select("sprint_id")
...
async def get_my_tasks(current_user: dict = Depends(get_current_user)):
    ...
    FIX 7: CCR tasks (mid_sprint_changed=true) are fetched separately and
    ...
    sentinel/wrong sprint_id (e.g. ffffffff-fe00-...) that doesn't exist in
    the sprints table and therefore doesn't match the sprint filter.
    ...
    # ── FIX 7: Always fetch CCR tasks separately and merge ───────────────────
    ...
    if project_id:
        try:
            ...
            if ccr_tasks:
                # Deduplicate: tasks already in the main result take precedence
...
async def update_task_status(
    ...
    if status == "done":
        ...
        # ── Resolve sprint_id if missing or sentinel ──────────────────────────
        # CCR tasks may carry a sentinel sprint_id; resolve the real one so the
        # adaptive engine and mid-sprint change fire against the correct sprint.
        ...
        if not sprint_id or is_sentinel:
            try:
                ...
                logger.error(f"[Tasks] Could not resolve sprint_id via mid_sprint_change: {e}")
        ...
        if not sprint_id or is_sentinel:
            ...
            if sprint_obj:
                ...
                if sprint_id:
                    ...
                    logger.info(f"[Tasks] Backfilled sprint_id={sprint_id} on task={task_id}")
